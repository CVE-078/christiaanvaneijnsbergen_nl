...
**No em dashes anywhere in the application.** Not in user-facing copy, not in code comments, not in new docs. Rework the sentence with a comma, period, semicolon, or parentheses, or use a plain hyphen where a dash truly reads best. This applies to every string a user can see and to anything you author in the repo. (Existing em dashes get cleaned up opportunistically when you touch a file, not in a big sweep.)
