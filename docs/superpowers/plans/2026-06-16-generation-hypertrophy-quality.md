...
Suite at branch HEAD: **1635 green**, typecheck clean.
...
## Remaining work
- **#3 isolation-ranking quality (next, ready).** Encode the score table above as a name-keyed quality map and wire it into the isolation branch of `byPattern`'s sort, mirroring the existing `CANONICAL_ANCHORS` pattern (keyed by exercise `name`). Synthetic-pool goldens stay byte-identical (no matching names), so the only change is the real catalogue stops defaulting to Kickback / Concentration Curl. Verify via `gen-routine.ts`. A future `quality` column next to `fatigue` is the long-term home (mirrors the `anchor_rank` follow-up).
- **#4-refinement.** Tighten `PUSH_PULL_RATIO_MAX` 2.0 → 1.5 in `programValidation.ts`; re-check the golden inputs don't newly trip it (the bucket change in #4 altered the ratios, so re-measure).
- **Tier-2: muscle-volume-aware generation (the big lever).** Wire the existing muscle bridge (`muscleMap.ts`, already used for reporting + the P3.2 priority volume) into: (a) muscle-based warnings (chest_underdosed / lateral_delts_low replacing pattern push/pull), (b) minimum-coverage gap-fill using the target table above, (c) muscle-based variety scoring (lat vs upper-back pulls without new patterns). Spec-first.
- **Deferred follow-ons:** exercise-specific rep floors (dumbbell loading); training style influencing exercise selection, not just reps (bodybuilding ≠ strength menu); estimator/band tuning vs real logged durations (#5-proper).
