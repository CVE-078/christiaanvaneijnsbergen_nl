...
## Remaining work
- **Tier-2: muscle-volume-aware generation (the big lever).** Decomposed into 3 specs (measure before intervene). **(a) muscle-based warnings: SHIPPED 2026-06-16** as Tier-2 Spec 1, a warn-only per-muscle volume layer on a new stored `primary_muscle` (NOT the coarse `muscleMap.ts` bridge, which cannot resolve delt heads / lats vs upper-back); spec `docs/superpowers/specs/2026-06-16-09-21-12-muscle-coverage-warnings-design.md`, plan `docs/superpowers/plans/2026-06-16-muscle-coverage-warnings.md`. **(b) muscle-based variety scoring: DROPPED on evidence** (the sweep `docs/audits/2026-06-16-10-49-42-muscle-coverage-evidence-sweep.md` found same-pattern clustering in 0/39 configs, so a `byPattern` reorder fixes a non-problem). **(c) minimum-coverage gap-fill: SHIPPED 2026-06-16** as Tier-2 Spec 3 (`feature/muscle-coverage-gapfill`), a capped post-generation isolation-only pass that closes the small-muscle volume gaps; spec `docs/superpowers/specs/2026-06-16-11-13-09-muscle-coverage-gapfill-design.md`, plan `docs/superpowers/plans/2026-06-16-muscle-coverage-gapfill.md`. The Tier-2 muscle-volume track is complete.
