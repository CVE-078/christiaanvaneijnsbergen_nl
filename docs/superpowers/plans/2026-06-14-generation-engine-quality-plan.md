...
- **#4 priority dose: RESOLVED (user pick) -> +4 + per-muscle ceiling.** Bumped `PRIORITY_EXTRA_SETS_PER_WEEK` 3 -> 4 and added `PRIORITY_MUSCLE_SET_CEILING` (20): the bump stops once the priority muscle's total weekly direct sets reach the ceiling, so it is meaningful (Claude.ai) without overshooting into junk volume (Perplexity). Null priority byte-identical; goldens hold.

Bundle B (deferred quad/ham + lower-day) - both lenses agreed, GO -> **SHIPPED** (see below).
...
The deferred composition call was resolved through the review loop (both lenses GO) and implemented: two new `quad_iso` / `hamstring_iso` MovementPatterns (types.ts + muscleMap `{ legs: 1 }` + ExerciseFormSheet `PATTERN_LABELS`), and the three split lower emphases gained dedicated knee-isolation slots: `lower_quad` and `lower_lean` drop `glute_iso` for `quad_iso` (glute returns via backfill), `lower_post` drops `lunge` for `hamstring_iso` (lunge returns via backfill, keeps `glute_iso`, still hinge-anchored). Same rep ranges across both leg days (no cross-day split). Migration `2026-06-15-10-55-31-leg-isolation-movement-patterns.sql` (hand-apply) re-tags Leg Extension -> `quad_iso` and Leg Curl / Dumbbell Leg Curl (Lying) -> `hamstring_iso` (is_compound unchanged), so the work the audit Cases 05/06/08/09/10 expected now lands in its dedicated slot instead of disappearing.
- **Goldens:** re-baselined the 3 affected byte-identity goldens (ul-classic-4, ulppl-5, ul-aesthetic-4); ppl-3 / ppl-fb-4 / fb-hmhp-4 (no split-lower emphasis) unchanged. Dependent tests updated for the new composition (glute-priority tilt, P2.1 deflection, GQ2 fresh-vs-used switched to `calf`, the dumbbell thin-lower floor pool). Suite 1621 green, typecheck clean.
- **Second-opinion review disposition:** the code-reviewer flagged role-bucket / validation / priority sets as "missing" the new patterns. Dismissed: those sets are for COMPOUNDS / anchors, and `quad_iso`/`hamstring_iso` are ISOLATION (the `_iso` suffix routes them to the ISOLATION role bucket, isolation rep range, and `{ legs: 1 }` attribution; goldens confirm 3x12-15 isolation placement). The one substantive note (auto-superset antagonist families exclude the new patterns) is an **accepted consequence** of the intentional single-region leg-day design: a quad-only or posterior-only day has no internal antagonist pair regardless, so a ~30-min split-leg day correctly runs solo sets. **Deferred polish:** adding `quad_iso`/`hamstring_iso` to `LOWER_BUCKET_FALLBACK` for consistency with `glute_iso` (rarely fires now that the dedicated slots fill in the first pass). The reviewer's valid systemic point - scattered hardcoded MovementPattern lists are fragile for future additions - is logged as tech debt, not a blocker.

## Pre-merge adversarial review (2026-06-15) - 7 findings, all fixed

A 6-lens adversarial review of the whole branch (find -> verify -> synthesize) ran before merge; migrations + validator wiring + new-pattern handling across muscleMap/labels/aggregators + duration_s downstream math all came back clean. Seven real findings the green suite missed were confirmed and fixed (suite 1629 green, typecheck clean):
- **B1 (blocker, commit f25371a):** the timed-hold logger reused `re.reps` (the numeric generated rep range, e.g. "15-20") instead of the hold range from `default_reps` ("30-60s"), so a generated Plank prefilled 15s and showed "hold 15-20s" while the Plan page correctly showed "30-60s hold". Added a `holdRange` prop (default_reps) threaded from both callers; `DEFAULT_HOLD` now exported as the fallback. The original tests passed the hold range AS repsRange, a state production never produces; added a regression test with numeric repsRange + separate holdRange.
- **B2 (blocker, commit 18281ed):** the Item 2 zero-compound guard could seat an off-contract squat on the hinge-anchored posterior day (the floor guard filtered `isOffContractLowerCompound`, Item 2 did not), reachable via hidden exercises + a restriction. Added the same off-contract filter; no-op on deep pools (all goldens byte-identical). New zero-compound repro test.
- **B3 (blocker, commit a1b8e18):** the priority +1 ceiling gated on a mid-stream running count, so a dense split (6-day advanced, chest baseline 21 > the 20 ceiling) still landed bumps and overshot to 24. Deferred the dose: accumulate the full-week baseline + bumpable rows during selection, distribute the +4 budget after the loop gated on the PROJECTED total (over-time estimate now runs post-bump too). Baseline-saturated muscle now gets 0 bumps. Null priority byte-identical; new dense-split regression test (21 -> stays 21).
- **N1 (commit a1b8e18):** `PRIORITY_PATTERNS.legs` omitted the new iso patterns; now `['squat','hinge','lunge','quad_iso','hamstring_iso']` so a legs priority doses the posterior day + direct isolation, like every sibling priority.
- **N2-N4 (commit 14fbf09, comment/label only):** relabeled the rebaselined ul-classic-4/ulppl-5 goldens (only ppl-3 still reproduces the pre-change golden); dropped the now-false "no quad_iso/hamstring_iso pattern" claims (PHUL keeps a hinge by design; the compound-first tiebreak is a no-op safety net for synthetic pools); corrected the lower_quad/lower_post/lower_lean "6th slot" comments (glute_iso does NOT return; the 6th is a 2nd quad_iso/lunge, and lower_post's lunge comes from the floor guard).

Deferred polish (not blocking): `LOWER_BUCKET_FALLBACK` iso-consistency; the systemic "scattered hardcoded MovementPattern lists" tech-debt; #8 estimator warmup front-loading.
