...
It is a UI / information-architecture redesign. No new domain logic in the generator, no migration. New work is presentational plus two small pure helpers and additive `explainCopy` concepts.
...
- **Phase description** (plain-language, muted, updates per phase) so a newcomer understands the phase:
...
## 9. Open questions for the review loop
