...
- WorkoutModeScreen redesign — the guided workout screen rebuilt responsively (frontend-design). **Mobile = "Focus"**: bold Big-Shoulders exercise name + set-progress ring, per-step progress pips, gradient Next/Finish CTA. **Desktop = two-pane**: a left focus pane (hero + set rows) and a right 392px "This session" sidebar with overall pips, a clickable whole-session exercise track (progress spine, jump-to-step), and a docked rest timer + sets-logged / sets-left stats + actions. Exactly one layout mounts via `useMediaQuery` (hardened against a missing `matchMedia`), keeping a single DOM and the full behavioural contract. Set rows stay `SetLogger` and the rest bar stays `RestTimer` (shared + tested); all guided-mode logic (auto-advance, superset-round gating, PR toast, mid-session swap, step clamp) is unchanged. Shipped after an adversarial multi-dimension review: AA-contrast pass on small labels (muted→dim) and accessible naming on the swap/track controls. (Deferred: guided-mode deload prefill; per-set "idle/active" focus states; the mockup's green "done" token, intentionally kept coral to follow the themeable accent.)
