...
- **Phase 0 — data foundations (highest leverage, no visible feature):** build the weighted MovementPattern ↔ muscle bridge (reconcile the draft map to Pulse's real 15 patterns; the 6 isolation patterns are already muscle-specific so attribution is near 1:1, the real work is the ~7 compounds' secondary muscles). **Decide muscle granularity first: keep the existing 10-category taxonomy and collapse the map into it (no analytics churn); expand to quad/ham + delt splits only if specialization later demands it.** Expand exercise metadata (primary/secondary muscles into the generation path + backfill, add `fatigue_cost`, `unilateral`, `joint_stress`, `difficulty`); add `session_id` + a workout date to `set_logs` so skip detection / adherence / behavior learning become trustworthy. Optionally tune `VOLUME_TARGETS` toward the research landmarks (the draft's MAV runs higher than Pulse's current bands for back / glutes / shoulders). **(Done 2026-06-06: raised the `exercise_swaps.week` cap 12 → 52.)**
