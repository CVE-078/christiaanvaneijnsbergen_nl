...
| # | Feature | Notes |
|---|---------|-------|
| 1 | Program start-day selection | Let users pick the program-week start day so a mid-week start isn't flagged "behind". Likely a `program_start_dow` on the routine driving the adherence window. Fixes a rough edge in adaptive regen. |
| 2 | Accent colour selection | Let the user pick their accent colour from a preset set; persist the choice and override the `pulse-accent` token at runtime. Low-effort personalisation polish. |
| 3 | WorkoutModeScreen redesign | Redesign the guided workout screen (use the frontend-design skill). |

_Auto-applied deload for stalled lifts + onboarding day-count fix shipped 2026-06-05 (see Shipped)._
