...
- Plan restructure — a `PageTitle` + Generate header, then a single program-header card (phase + week notes + rationale, with an inline week stepper) replacing the separate phase card / WeekSelector / "Why this plan" blocks, then a desktop two-column grid: weekly schedule + a taller, two-label volume chart on the left, the per-session exercise breakdown on the right
- Library unification — one shared toolbar (filter chips + actions in a single row) and one row-card style across the Exercises, Routines, and Templates tabs, plus a `PageTitle`, so the three tabs read as one library instead of three different layouts
