...
function BodyweightChart({ entries, unit }: { entries: BodyweightEntry[]; unit: Unit }) {
...
export default function ProfileView({ email, displayName, unit, bodyweightLogs, onUnitChange, onDisplayNameChange, onBodyweightLogsChange }: Props) {
  ...
  function handleUnitChange(newUnit: Unit) {
    ...
    startTransition(async () => { await updateProfile(displayName, newUnit); });
  }
  ...
  function handleLogBodyweight() {
    ...
    startTransition(async () => {
      try {
        const entry = await logBodyWeight(kgVal);
        const deduped = bodyweightLogs.filter(e => e.logged_at !== entry.logged_at);
        const updated = [entry, ...deduped].sort((a, b) => b.logged_at.localeCompare(a.logged_at));
        onBodyweightLogsChange(updated);
        setBwInput('');
      } catch {
  ...
  function handleDeleteBodyweight(id: string) {
    const snapshot = bodyweightLogs;
    onBodyweightLogsChange(snapshot.filter(e => e.id !== id));
    startTransition(async () => {
      try {
        await deleteBodyWeight(id);
      } catch {
        onBodyweightLogsChange(snapshot);
      }
    });
  }
    ...
    <div style={{ padding: '1.25rem 1rem 3rem', maxWidth: 480, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
        <div style={{
          ...
          fontFamily: MONO, fontSize: '1.25rem', fontWeight: 700, color: ACCENT, letterSpacing: '-0.02em',
        }}>
          {initials}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          {editingName ? (
            <input
              ...
              style={{
                fontFamily: MONO, fontSize: '0.9375rem', fontWeight: 600, color: '#fff',
                background: 'transparent', border: 'none', borderBottom: `1px solid ${ACCENT}`,
            ...
            <button
              ...
              style={{
                fontFamily: MONO, fontSize: '0.9375rem', fontWeight: 600,
                color: displayName ? '#fff' : DIM,
                background: 'none', border: 'none', padding: 0,
                cursor: 'text', textAlign: 'left', display: 'block', width: '100%',
              }}
            >
      ...
      {/* Body weight */}
      <div>
        <div style={{ fontFamily: MONO, fontSize: '0.5625rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: MUTED, marginBottom: '0.75rem' }}>
          Body Weight
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start', marginBottom: '0.875rem' }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              <input
                ...
                style={{
                  width: '5.5rem', padding: '0.375rem 0.5rem',
                  background: '#0a0a0a', border: `1px solid ${bwError ? '#f43f5e' : BORDER}`,
                  borderRadius: 3, color: '#fff', fontFamily: MONO, fontSize: '0.8125rem', outline: 'none',
                }}
              />
              <span style={{ fontFamily: MONO, fontSize: '0.6875rem', color: DIM }}>{today}</span>
            </div>
            {bwError && (
              <div style={{ fontFamily: MONO, fontSize: '0.625rem', color: '#f43f5e', marginTop: '0.25rem' }}>
                {bwError}
              </div>
            )}
          </div>
          <button
            ...
            style={{
              ...
              border: `1px solid ${MUTED}`, borderRadius: 3, color: '#aaa',
              cursor: isPending ? 'not-allowed' : 'pointer', opacity: isPending ? 0.5 : 1, flexShrink: 0,
            }}
          >
            Log
          </button>
        ...
        {bodyweightLogs.length > 0 ? (
          <div>
            {bodyweightLogs.map(entry => (
              <div
                key={entry.id}
                style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.4375rem 0', borderBottom: '1px solid #111' }}
              >
                <span style={{ fontFamily: MONO, fontSize: '0.6875rem', color: DIM, flex: 1 }}>
                  {entry.logged_at}
                </span>
                <span style={{ fontFamily: MONO, fontSize: '0.8125rem', color: '#d4d4d4', fontWeight: 600 }}>
                ...
                <button
                  ...
                  style={{ fontFamily: MONO, fontSize: '0.625rem', color: '#444', background: 'none', border: 'none', cursor: 'pointer', padding: 0, flexShrink: 0 }}
                >
                  ✕
                </button>
              </div>
